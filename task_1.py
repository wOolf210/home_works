...
async def create_file_with_random_number_async(file_name):
...
def run_with_delay():
    ...
    for i in range(1000):
        ...
        create_file_with_random_number(file_name)
...
def run_with_threads():
    ...
    for i in range(1000):
        file_name = f"file_{i}.txt"
        thread = threading.Thread(target=create_file_with_random_number, args=(file_name,))
...
async def run_with_async():
    ...
    for i in range(1000):
        file_name = f"file_{i}.txt"
        task = create_file_with_random_number_async(file_name)
        tasks.append(task)
        file_names.append(file_name)
